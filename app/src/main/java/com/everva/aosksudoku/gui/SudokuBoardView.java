...
import com.everva.aosksudoku.game.CellCollection.OnChangeListener;
...
//        Log.d(TAG, "heightSize=" + heightSize);
		...
		Typeface plain = Typeface.createFromAsset(getContext().getAssets(), "font/o_symb.ttf");

		float cellTextSize = mCellHeight * 0.75f;
		mCellValuePaint.setTextSize(cellTextSize);
		mCellValuePaint.setTypeface(plain);
		mCellValueReadonlyPaint.setTextSize(cellTextSize);
		mCellValueReadonlyPaint.setTypeface(plain);

		mCellValueInvalidPaint.setTextSize(cellTextSize);
		mCellValueInvalidPaint.setTypeface(plain);

		mCellNotePaint.setTextSize(mCellHeight / 3.0f);
		mCellNotePaint.setTypeface(plain);
